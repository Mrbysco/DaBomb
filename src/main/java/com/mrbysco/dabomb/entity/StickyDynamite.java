...
public class StickyDynamite extends ThrowableItemProjectile {

	public StickyDynamite(EntityType<? extends StickyDynamite> entityType, Level level) {
	...
	public StickyDynamite(PlayMessages.SpawnEntity spawnEntity, Level level) {
		this(BombRegistry.STICKY_DYNAMITE.get(), level);
	}
	...
	public void tick() {
		...
		if (tickCount >= 90 && (this.isNoGravity() || isPassenger())) {
			if (!this.level.isClientSide) {
				this.explode();
	...
	protected void explode() {
		this.level.explode(this, this.getX(), this.getY(0.0625D) + 0.5F, this.getZ(), BombConfig.COMMON.stickyDynamiteRadius.get().floatValue(), Explosion.BlockInteraction.BREAK);
	}
